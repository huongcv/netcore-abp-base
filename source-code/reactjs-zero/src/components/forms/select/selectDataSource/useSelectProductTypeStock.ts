...
export const useSelectProductTypeStock = (shopType?: ShopType): SelectDataSource => {
    const {t} = useTranslation('comboEnum');
    const key = 'ProductTypeStock_';//+ shopType;
    return useSelectDataSource(key, async () => {
        const result = await ComboEnumService.getProductTypeStock()
