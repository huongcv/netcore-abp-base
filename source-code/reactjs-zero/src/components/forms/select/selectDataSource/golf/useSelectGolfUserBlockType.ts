...
export const useSelectGolfUserBlockType = (): SelectDataSource => {
    const key = 'useSelectGolfUserBlockType';
    const {t} = useTranslation('enum');
    return useSelectDataSource(key, async () => {
        const result = await ComboEnumService.getGolfUserBlockTypeEnum()
