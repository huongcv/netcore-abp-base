...
interface SelectEmployeeToAssignProps {
    userId: string;
}
export const useSelectEmployeeToAssign = (
    props: SelectEmployeeToAssignProps
): SelectDataSource => {
    const key = "PartnerEmployeeAssign_" + (props.userId ?? "0");

    return useSelectDataSource(key, async () => {
        const result = await EmployeeService.getComboOptionForAssignUser({
            body: {
                userId: props.userId,
            },
        });
...
export const EmployeeOptionLabel = (props: { dto: PartnerDto }) => {
    const { dto } = props;
    return (
        <>
            {dto && (
