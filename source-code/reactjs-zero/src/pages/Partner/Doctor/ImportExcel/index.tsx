...
import {DoctorDataTableExcel} from "@pages/Partner/Doctor/ImportExcel/tableData";
...
const ImportExcelDoctor = () => {
    ...
    const dataExcel = useReadExcelToImportPartner(binaryStrExcel, 6);
    ...
    const validateData = async (items: any[]) => {
        try {
            UiUtils.setBusy();
            const validateData = await PartnerDoctorService.validateImport({
                body: items
            });
            UiUtils.clearBusy();
            setMessage('');
    ...
    const handlerDownloadTemplate = async () => {
        try {
            UiUtils.setBusy();
            const [blobResult] = await Promise.all([PartnerDoctorService.exportTemplate({
                responseType: 'blob'
            })]);
    ...
    return (<>
        <div
            ...
            <OrdBreadcrumb mainTitle={'menu.importDoctor'} itemsRoute={itemsRoute}></OrdBreadcrumb>
                                      ...
                                      items={[
                                          {
                                              ...
                                              children: <DoctorDataTableExcel datasource={validList} isValid={true}
                                          ...
                                          },
                                          {
                                              ...
                                              children: <DoctorDataTableExcel datasource={invalidList}
...
export default observer(ImportExcelDoctor);
