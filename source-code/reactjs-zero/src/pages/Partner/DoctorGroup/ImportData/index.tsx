...
const ActionExcelBtnProductGroup: React.FC = () => {
  ...
  const { t } = useTranslation('doctor-group');
  ...
  const dataExcel = useReadExcelToImportPartnerGroup(binaryStrExcel, 6);


  const handleDownloadTemplate = async () => {
    try {
      const response = await DoctorGroupService.exportTemplate({ responseType: "blob" });
  ...
  const handleValidateData = async (items: any[]) => {
    try {
      UiUtils.setBusy();
      const { successImportList, errorImportList, errorFile } = await DoctorGroupService.validateImport({
        body: items,
      });
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between mb-3">
        <OrdBreadcrumb mainTitle={`menu.actionExcelBtnProductGroup.6`} itemsRoute={[{ title: "menu.partnerDoctor", route: "/partner/doctor" }]} />
        <Space wrap>
      ...
      <Card title={`${t("TextTitleImportBy")} ${t(`partnerGroupLowerCase.6`)}`}>
        <Row>
          <Col span={24}>
            <ImportExcelButton
