...
const ActionExcelBtnProductGroup: React.FC = () => {
  ...
  const dataExcel = useReadExcelToImportPartnerGroup(binaryStrExcel, 2);


  const handleDownloadTemplate = async () => {
    try {
      const response = await SupplierGroupService.exportTemplate({ responseType: "blob" });
  ...
  const handleValidateData = async (items: any[]) => {
    try {
      UiUtils.setBusy();
      const { successImportList, errorImportList, errorFile } = await SupplierGroupService.validateImport({
        body: items,
      });
  ...
  return (
    <>
      <div className="flex flex-wrap items-center justify-between mb-3">
        <OrdBreadcrumb mainTitle={`menu.actionExcelBtnProductGroup.2`} itemsRoute={[{ title: "menu.customerSupplier", route: "/partner/customer-supplier" }]} />
        <Space wrap>
      ...
      <Card title={`${t("TextTitleImportBy")} ${t(`partnerGroupLowerCase.2`)}`}>
        <Row>
          <Col span={24}>
            <ImportExcelButton
