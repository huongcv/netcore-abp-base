...
import {SupplierDataTableExcel} from "@pages/Partner/CustomerSupplier/ImportExcel/tableData";
...
const itemsRoute: IItemRoute[] = [
    {
        ...
        route: '/partner/customer-supplier'
    }
]
...
const ImportExcelSupplier = () => {
    ...
    const dataExcel = useReadExcelToImportPartner(binaryStrExcel, 2);
    ...
    const validateData = async (items: any[]) => {
        try {
            ...
            UiUtils.setBusy();
            const validateData = await SupplierService.validateImport({
                body: items
            });
            UiUtils.clearBusy();
            setMessage('');
    ...
    const handlerDownloadTemplate = async () => {
        try {
            UiUtils.setBusy();
            const [blobResult] = await Promise.all([SupplierService.exportTemplate({
                responseType: 'blob'
            })]);
    ...
    return (<>
        <div
            ...
            <OrdBreadcrumb mainTitle={'menu.importSupplier'} itemsRoute={itemsRoute}></OrdBreadcrumb>
                                      ...
                                      items={[
                                          {
                                              ...
                                              children: <SupplierDataTableExcel datasource={validList} isValid={true}
                                          ...
                                          },
                                          {
                                              ...
                                              children: <SupplierDataTableExcel datasource={invalidList}
