...
import {HotelIcon} from "@ord-components/icon/HotelIcon";
...
const RevenueByChanelCard = () => {
    ...
    const ShowChannelName = (prop: { type?: CHANNEL_TYPE }) => {
        ...
        switch (prop.type) {
            case 0: {
                _channelName = 'Nhà hàng'
                _channelGroupName = 'Bán trực tiếp cửa hàng'
                const Icon = () => {
                    return <StoreIcon/>
                }
                break;
            ...
            // sàn tmđt
            case 1: {
                _channelName = 'Khách sạn'
                _channelGroupName = 'Sàn thương mại điện tử'
                break;
            }
            case 2: {
                _channelName = 'Sân Golf'
                _channelGroupName = 'Mạng xã hội'
                break;
            }
            case 3: {
                ...
                <div className={'container-revenue-chanel w-full'}>
                    {getChannelData(ALL_CHANNEL_TYPES, data).map((channel, index) => (
                        <div className={'child-revenue-chanel'} key={index}>
                            ...
                            <p className={'revenue-money'}>{formatterNumber(channel.amount, 0)} {currencyDefault} VNĐ</p>
