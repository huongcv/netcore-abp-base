...
import CreateOrUpdateFormFnbProcessingArea from "./CreateOrUpdateFormFnbProcessingArea";

const FnbProcessingArea = () => {
  const {
    fnbProcessingAreaStore: mainStore,
  } = useStore();
  ...
  return (
    <>
      <OrdCrudPage
        ...
        columns={columns}
        contentTopTable={
          <IsActiveStatusCounter getCountApi={FnbProcessingAreaService.getCount} />
        }
        searchForm={(f) => (
          <>
            <SearchFilterText
      ...
      <OrdCreateOrUpdateModal
        stored={mainStore}
        entityForm={() => <CreateOrUpdateFormFnbProcessingArea />}
        onSavedSuccess={() => {
          mainStore.refreshGridData()
        }}
      />
...
export default observer(FnbProcessingArea);
