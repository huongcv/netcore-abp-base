...
import {ImportExcelExportExcelBaseDto, MOVE_TYPE} from "@api/index.defs";
...
import {InvalidModalExportCancelStock} from "@pages/StockManagement/ExportCancel/ImportExcel/InvalidModal";

const ImportExcelExportCancel = () => {
    ...
    const handlerDownloadTemplate = async () => {
        try {
            UiUtils.setBusy();
            const blobResult = await StockValidateExcelService.getStockTemplate({moveType: MoveType.PhieuXuatHuy}, {
                responseType: 'blob'
            });
            FileSaver.saveAs(blobResult, "Phieu-xuat-huy-mau.xlsx");
        } catch (ex: any) {
            console.error(ex);
        } finally {
            UiUtils.clearBusy();
    ...
    const validateData = async () => {
        ...
        try {
            const {data, isSuccessful, message} = await StockValidateExcelService.validateDataExportStock({
                uploadFile: file,
    ...
    const addValidItem = (data: ImportExcelExportExcelBaseDto[]) => {
    ...
    return (
        <>
            ...
            <div className={'text-center'}>
                ...
                <InvalidModalExportCancelStock moveType={MoveType.PhieuXuatHuy} isModalOpen={isOpen} setIsModalOpen={setIsOpen}
                                           invalidList={invalidList}
                                           validList={validList}></InvalidModalExportCancelStock>
            </div>
        </>

    );
}
export default observer(ImportExcelExportCancel);
