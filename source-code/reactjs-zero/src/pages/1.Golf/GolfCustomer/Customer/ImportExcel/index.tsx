...
import {CustomerDataTableExcel} from "./tableData";
...
import useReadExcelToImportPartner from "@pages/Partner/UploadExcel/useReadExcelToImportPartner";
...
const itemsRoute: IItemRoute[] = [
    {
        title: 'menu.customer',
        route: '/app/golf/customer'
    }
]

const ImportExcelCustomer = () => {
    ...
    const dataExcel = useReadExcelToImportPartner(binaryStrExcel, 1);
    ...
    const validateData = async (items: ImportPartnerInputDto[]) => {
        try {
            UiUtils.setBusy();
            const validateData = await CustomerService.validateImport({
                body: items
            });
            UiUtils.clearBusy();
            setMessage('');
    ...
    const handlerDownloadTemplate = async () => {
        try {
            UiUtils.setBusy();
            const [blobResult] = await Promise.all([CustomerService.exportTemplate({
                responseType: 'blob'
            })]);
    ...
    return (<>
        <div
            ...
            <OrdBreadcrumb mainTitle={'menu.importCustomer'} itemsRoute={itemsRoute}></OrdBreadcrumb>
                                      ...
                                      items={[
                                          {
                                              ...
                                              children: <CustomerDataTableExcel datasource={validList} isValid={true}
                                          ...
                                          },
                                          {
                                              ...
                                              children: <CustomerDataTableExcel datasource={invalidList}
...
export default observer(ImportExcelCustomer);
