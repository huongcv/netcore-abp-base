...
function AddProductCard(props: {
    ...
    function onSave() {
        UiUtils.setBusy();
        mainStore.saveProductTemplate()
            .then(res => {
                UiUtils.clearBusy();
                if (res.isSuccessful) {
                    mainStore.afterSaveProductTemplate()
                    props.onClose();
                } else {
    ...
    return (
        <Card
            className={`xl:w-80 lg:w-60 md:w-60 sm:w-60 flex-shrink-0 h-full mr-1`}
            size={"small"}
            style={{
                borderRadius: 0,
            ...
            extra={<>
                <Button size='small' className='w-6' onClick={() => {
                    ...
                    Object.entries(mainStore.productAddTempData).forEach(([key, value]) => {
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {items.map((item) => (
                                    <div
                                        key={item.id}
                                        onClick={() => {
                                            mainStore.addProductToTemp(item);
                                        }}
