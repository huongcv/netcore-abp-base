...
import withPricePaymentBookingGroupInfo
    from "@pages/1.Golf/TeeSheet/Booking/Crud/GroupBooking/withPricePaymentBookingGroupInfo";
import {SelectDataSource} from "@ord-components/forms/select/selectDataSource/selectDataSource";
import {DiscountTypeEnum} from "@ord-core/enum/discountTypeEnum.enum";

const PaymentBookingGroupInfo = (props: {}) => {
    const {
    ...
    const depositPercentage_w = Form.useWatch("depositPercentage", form);
    ...
    const [sourceDepositPercent] = useState<SelectDataSource>({
        data: [
            {
                value: 30,
                label: "30%"
    ...
    useEffect(() => {
        if (paymentMode_w == PaymentModeEnum.Deposit) {
            form.setFieldValue("depositAmount", totalAmount_w * (depositPercentage_w / 100));
        } else {
            form.setFieldValue("depositAmount", 0);
        }
    }, [totalAmount_w, depositPercentage_w, paymentMode_w]);
    return (
        <>
            ...
            <Card title={t('paymentInfo')}
                ...
                <Row gutter={[16, 8]}>
                    ...
                    {paymentMode_w === PaymentModeEnum.Deposit && <>
                        <ColSpanResponsive span={24}>
                            <FloatLabel label={t('depositPercentage')}>
                                <Form.Item name='depositPercentage' initialValue={30}>
                                    <OrdSelect datasource={sourceDepositPercent}></OrdSelect>
                                </Form.Item>
                            </FloatLabel>
                        </ColSpanResponsive>
                        <ColSpanResponsive span={24}>
                            <FloatLabel label={t('depositAmount')}>
                                <Form.Item name='depositAmount' className='text-right'>
                                    <PriceNumberInput disabled></PriceNumberInput>
                                </Form.Item>
                            </FloatLabel>
                        </ColSpanResponsive>
                    ...
                    {/*    </FloatLabel>*/}
                    {/*</ColSpanResponsive>*/}
                    {paymentMode_w !== PaymentModeEnum.PayLater && <ColSpanResponsive span={24}>
...
PaymentBookingGroupInfo.propTypes = {};

export default observer(withPricePaymentBookingGroupInfo(PaymentBookingGroupInfo));
