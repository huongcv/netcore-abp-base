...
import {ProductFromInventoryWithUnitDto, ProductSearchWithUnitDto, ShopTemplateDetailsDto} from "@api/index.defs";
...
const SearchProductTableModal = (props: {
    onItemsSelected?: (items: ProductFromInventoryWithUnitDto[]) => void,
}) => {
    const {t} = useTranslation('stock');
    const {reportPharmacyLogQualityInspectionSearchProductStore: searchProductStore} = useStore();
    const [tabActiveKey, setTabActiveKey] = useState('1');
    const {isModalOpen} = searchProductStore;
    const columns = [
        ...getProductColumns()
    ];
    const rowSelection: TableProps<ProductFromInventoryWithUnitDto>['rowSelection'] = {
        selectedRowKeys: searchProductStore.selectedRowKeys,
        onChange: (selectedRowKeys: React.Key[], selectedRows: ProductFromInventoryWithUnitDto[]) => {
            console.log("selectedRows", selectedRows)
            searchProductStore.setSelectedRows(selectedRows);
        },
        getCheckboxProps: (record: ProductFromInventoryWithUnitDto) => ({
