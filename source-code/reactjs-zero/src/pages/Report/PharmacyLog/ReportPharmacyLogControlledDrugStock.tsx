...
function ReportPharmacyLogControlledDrugStock() {
  const { reportPharmacyLogControlledDrugStockStore: stored } = useStore();
  ...
  const columns: TableColumnsType<any> = TableUtil.getColumns(
    [
      ...
      },
      {
        ...
        align: "center",
        width: 120,
        render: (_, __: PharmacyLogReportControlledDrugStockOutputDto) =>
          _ ? (
            <>
              {__.status == 3 && (
      ...
      },
      {
        ...
        children: [
          ...
          {
            title: <div className="text-center">{t("shrinkage")}</div>,
            dataIndex: "shrinkage",
            width: 100,
            align: "right",
          },
  ...
  const DataTable = (props: any) => {
    const { currentPageResult: summaryData } = stored as {
      currentPageResult: PharmacyLogReportPageListControlledDrugStockOutputDto;
    };
    return (
      <>
        <Table
              ...
              <Table.Summary fixed="top">
                <Table.Summary.Row>
                  ...
                  <Table.Summary.Cell
                    ...
                    className="font-bold !pr-1.5"
                  >
                    {formatterNumber(summaryData?.errorLoss, 0)}
                  </Table.Summary.Cell>
                  <Table.Summary.Cell
                    index={10}
                    align="right"
                    className="font-bold !pr-1.5"
                  >
                    {formatterNumber(summaryData?.shrinkage, 0)}
                  </Table.Summary.Cell>
      ...
      >
        <Row gutter={16}>
          ...
          <Col {...useResponsiveSpan(12)}>
            <SearchFilterText onReset={onResetClick} placeHolder={t("filterPlaceholder")} span={24} />
          </Col>
        </Row>
      </Form>
...
export default observer(ReportPharmacyLogControlledDrugStock);
