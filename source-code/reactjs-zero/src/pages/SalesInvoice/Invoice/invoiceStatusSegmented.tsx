...
import {InvoiceStatusPagedCountDto, SaleInvoiceGetListDto} from "@api/index.defs";

export const  InvoiceStatusSegmented = (props: {
    getCountApi(
        params: {
        ...
        try {
            ...
            const options = items.map(it => {
                if (it.status === null) {
                    return {
                        label: <Space wrap>
                            <span>{tEnum('SaleInvoiceStatus.All')}</span>
                ...
                return {
                    label: <>
                        <Space wrap>
                            <span>{
                                tEnum('SaleInvoiceStatus.' + it.status)
                            }</span>
                            <Badge count={it.totalCount} showZero overflowCount={Number.MAX_VALUE}></Badge>
                        </Space>
                    </>,
