    ...
    const handlerChangeUnit = (v: any, item: any) => {
        ...
        if (moveTypes.includes(moveType)) {
            ...
            if (moveType === MoveType.PhieuNhapNhaCungCap || moveType === MoveType.PhieuNhapTon) {
                form.setFieldValue([StockMoveFormName.ProductItems, field.name, 'price'], item?.data?.convertRate * (latestImportPrice_w || 0));
            } else {
                //gia = gia von * ty le quy doi
                form.setFieldValue([StockMoveFormName.ProductItems, field.name, 'price'], item?.data?.convertRate * (costPrice_w || 0));
            }
        }
    }
